...
def setup_game():
    ...
    if not os.path.exists(model_path):
        ...
        try:
            ...
            logging.info(f"Starting gesture model training with script: {training_script}")
            
            # Run the training script with a timeout to prevent hanging
            try:
                ...
                if result.stderr:
                    logger.log_error(Exception(result.stderr), "Model training stderr")
                
                if result.returncode == 0:
                    logger.log_game_event("model_training_complete", {"timestamp": time.time()})
                    logging.info("Model training completed successfully")
                else:
                    logging.warning(f"Model training failed with return code {result.returncode}")
                    logging.warning("Continuing with default controls. Some features may be limited.")
            
            except subprocess.TimeoutExpired:
                logger.log_error(Exception("Model training timed out after 5 minutes"), "Model training")
            except Exception as e:
                logger.log_error(Exception(f"Error during model training: {str(e)}"), "Model training")
            
            # Ensure we have a model file to prevent repeated training attempts
            if not os.path.exists(model_path):
                logging.info("Creating dummy model file to prevent repeated training attempts")
                try:
                    with open(model_path, 'w') as f:
                        f.write("dummy_model")
                except Exception as e:
                    logger.log_error(Exception(f"Failed to create dummy model file: {str(e)}"), "Model Training")
        
        except Exception as e:
            logger.log_error(Exception(f"Unexpected error during model training setup: {str(e)}"), "Model training")
            logging.warning("Continuing with default controls. Some features may be limited.")
