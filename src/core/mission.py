...
import math
import numpy as np
from src.core.game_config import MISSIONS, SCORE_MULTIPLIERS
...
class Mission:
    ...
    def get_status(self):
        # Add more detailed status for HUD
        return {
            'type': self.type,
            ...
            'objectives': [
                {
                    ...
                    'desc': self._objective_desc(obj)
                } for obj in self.objectives
            ],
            'feedback': self._get_feedback()
        }

    def _objective_desc(self, obj):
        if obj['type'] == 'park':
            return 'Park in the highlighted spot.'
        elif obj['type'] == 'speed':
            return f"Maintain {obj['target_speed']} km/h for {obj['duration']}s."
        elif obj['type'] == 'distance':
            return f"Drive {obj['target_distance']} meters."
        elif obj['type'] == 'collect':
            return 'Collect the cargo.'
        elif obj['type'] == 'deliver':
            return 'Deliver the cargo to the destination.'
        return ''
    ...
    def _get_feedback(self):
        ...
        for obj in self.objectives:
            if not obj['completed']:
                return self._objective_desc(obj)
        return 'Keep going!' 