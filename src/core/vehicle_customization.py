...
class Vehicle:
    ...
    def get_stats_string(self):
        ...
        return f"Speed: {stats['speed']:.0f}  Accel: {stats['acceleration']:.1f}  Handling: {stats['handling']:.2f}  Braking: {stats['braking']:.1f}  Fuel: {stats['fuel_efficiency']:.2f}"
    
    def upgrade_with_animation(self, upgrade_type: str):
        upgraded = self.upgrade(upgrade_type)
        # Return new stats if upgraded, for UI to display
        if upgraded:
            return self.get_stats_string()
        return None
    def get_upgrade_cost(self, upgrade_type: str) -> int:
        current_level = self.upgrades.get(upgrade_type, 0)
        return self._get_upgrade_cost(upgrade_type, current_level + 1)
    def can_afford_upgrade(self, upgrade_type: str) -> bool:
        return self.coins >= self.get_upgrade_cost(upgrade_type)
...
class VehicleManager:
    ...
    def get_selected_vehicle_stats_string(self):
        vehicle = self.get_selected_vehicle()
        if vehicle:
            return vehicle.get_stats_string()
        return "" 