...
class GestureController:
    ...
    def _load_calibration(self):
        ...
        try:
            ...
            if os.path.exists('gesture_calibration.json'):
                with open('gesture_calibration.json', 'r') as f:
                    content = f.read().strip()
                    if content:  # Only try to load if file is not empty
                        self.calibration_data.update(json.loads(content))
                        return
            
            # If we get here, either file doesn't exist or is empty
            self._save_calibration()
            
        except (json.JSONDecodeError, Exception) as e:
            # If there's an error loading the file, log it and use defaults
            logging.warning(f"Error loading calibration data: {e}. Using default calibration.")
            # Save the default calibration to repair the file
            self._save_calibration()
